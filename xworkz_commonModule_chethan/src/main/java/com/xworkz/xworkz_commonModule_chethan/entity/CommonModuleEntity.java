...
@NamedQuery(name = "getValue", query = "SELECT cme FROM CommonModuleEntity cme WHERE cme.email = :e" + "mail")
@NamedQuery(name = "updatePasswordAndCountByEmail",query = "UPDATE CommonModuleEntity cme SET cme.password = :password, cme.signinCount = :signinCount WHERE cme.email = :email")
@NamedQuery(name = "updateDetailsByName",query = "UPDATE CommonModuleEntity cme SET cme.ph =:ph, cme.alterEmail =:alterEmail, cme.alterPh =:alterPh, cme.location =:location,cme.email=:email WHERE cme.name =:name")
